...
- **万能函数思想的家谱**：神经网络不是凭空出现的。1715 泰勒(多项式基)→ 1807 傅立叶(sin/cos 基,一切信号 = sin/cos 叠加)→ 1885 Weierstrass(多项式稠密性)→ 1990s 小波(多尺度自相似基)→ 1989 UAT(神经元基)。**所有万能函数 = 基 × 系数 的线性组合**;神经网络的关键飞跃是**基函数本身从数据学出来**(其他都是预先定死)。可跑代码:用泰勒展开拟合 sin、用 20 项 sin 叠加出方波。**Fourier 思想在 Transformer 里仍然活着**:原版 sinusoidal 位置编码、RoPE 的复平面旋转、attention = "在学出来的 K 基上展开 V"
...
- **QKV 三角投影 + 多头切片 = "矩阵揉碎重组"**：同一个 token 被三套 W 撕成 Q/K/V 三种角色，再切成 12 段子空间独立搜索（`model.py:53-56`）。这就是为什么 Python 的 `for` 和 JS 的 `for` 在某些头里收敛到同一个"循环理想型"——**跨语言迁移的根源是揉碎重组**
- **GPT 是抄答案拼接器：迁移和幻觉是同一枚硬币**：`y = att @ v` 就是"加权拷贝历史 token 的 V"。邻居池密 → 举一反三；邻居池空 → softmax 强行归一 → 编造不存在的 API / 引用 / 数字。配一张"邻居密度 → 输出表现"的四档表
- **多头 = 多个特征矩阵**：K 就是"学到的特征模板"，和 CNN 卷积核同构；12 个头 = 12 套独立特征矩阵，靠不同梯度路径 + 随机初始化**自动分工**——这就是 QKV / Transformer 设计的精髓
- **子空间分离的机制：SVM 升维 vs Transformer 升维**：SVM 用核函数升维 + 显式超平面切；Transformer 在每个 Block 里做两次升维（`c_attn` 768→2304、`c_fc` 768→3072）+ 软检索 + 投影回来。**多头之间没有显式边界**——靠 `view` 的物理切片 + 训练涌现，是隐式分离。一张总图把 SVM / MLP / Attention / Block 排成"升维 + 高维操作 + 投影"的同一谱系
- **`top_k` / `top_p` 就是搜索引擎的 LIMIT**（`model.py:287-299`）：一个是硬 LIMIT，一个是动态自适应 LIMIT
- **训练个人 GPT = 在你的语料上重建索引**：`prepare.py` 是建索引，`F.cross_entropy`（`model.py:192`）是让"召回准"的反馈信号，FIM 是给搜索加上"中间填空"维度；10M 小模型在你私有代码上的内部 API 准确率可以超过 GPT-4——因为那块邻居密度只有你的训练能填
