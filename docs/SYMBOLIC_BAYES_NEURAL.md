...
6. ["训练数据 = 一堆可执行的符号代码"——这个类比对在哪、错在哪](#6-训练数据--一堆可执行的符号代码这个类比对在哪错在哪)
7. [三种范式的对照表](#7-三种范式的对照表)
8. [在 CodeGPT 里看见三个范式的影子](#8-在-codegpt-里看见三个范式的影子)
9. [神经-符号混合：现代系统都不是纯种](#9-神经-符号混合现代系统都不是纯种)
10. [给用户原问题的最终回答](#10-给用户原问题的最终回答)
...
2. **不可解释**——你不能指着某个权重说"这是关于 Python 缩进的规则"，它弥散在整个矩阵里。
