...
> 为什么大模型要先 SFT 再 RL？最后用模型做 QA / 对话时，这些训练是怎么"生效"的——是纯靠 Transformer 里的参数在预测吗？还是说实际对话时还需要一段配套代码，配合之前 SFT / RL 训练的结果才能跑起来？具体生效的机制是什么？
>
> 这篇文档回答三件事：
>
...
疑问🤔——"使用大模型时是纯靠参数预测吗？还是需要代码配合？"——其实是在问这张图的右半边。右半边做的几乎全部是"代码"：把 prompt 变成 `idx`、跑 `generate` 循环、判断停止条件、把 token 解码回字符串。这些代码如果和训练时的约定不一致，`W` 里训练好的能力就**激活不了**。
...
这一节回答第二个疑问🤔："这些机制是如何生效的？纯靠记录在 transformer 里的参数来预测吗？"
...
这正好回答了疑问🤔"策略梯度参数怎么体现在推理中"——**它不以"参数"的形式体现，它以"actor 权重被改动过"的形式体现**。策略梯度是一个**动词**（一种更新 `W` 的方式），不是一个**名词**（一组要在推理时读取的参数）。
...
> **疑问🤔：语义拟合的参数子空间是怎样一个过程？能不能把"从点积拟合，到最终回答问题"中间发生的事可视化出来？**
